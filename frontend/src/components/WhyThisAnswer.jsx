        ...
        <div className="px-5 pb-5 border-t border-slate-200 pt-4 space-y-4">
          <div className="space-y-3">
            <div>
              ...
              <p className="text-[14px] text-slate-800 font-medium">{message.product || t(language, 'j_not_verified')}</p>
            </div>
            <div>
              <p className="text-[11px] text-[#1C4E80] font-bold uppercase tracking-wider">{t(language, 'applicable_standard')}</p>
              <p className="text-[14px] text-slate-800 font-medium">{message.applicable_standard || t(language, 'j_not_verified')}</p>
            </div>
            <div>
              <p className="text-[11px] text-[#1C4E80] font-bold uppercase tracking-wider">Reason</p>
              <p className="text-[14px] text-slate-800 font-medium">{message.why_applicable || (message.structured_compliance_journey && message.structured_compliance_journey.summary) || t(language, 'j_not_verified')}</p>
            </div>

            {message.official_sources && message.official_sources.length > 0 && (
              <>
                <div>
                  <p className="text-[11px] text-[#1C4E80] font-bold uppercase tracking-wider">Evidence</p>
                  <p className="text-[14px] text-slate-800 font-medium">Official source: BIS</p>
                </div>
                {message.verified === true && (
                  <div>
                    <p className="text-[11px] text-[#1C4E80] font-bold uppercase tracking-wider">Source Status</p>
